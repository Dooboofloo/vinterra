...
execute if score #agg_rank vin.warmth_tmp > #aggregate_limit vin.warmth_meta run return 1
...
scoreboard players operation #agg_max vin.warmth_tmp > @e[type=marker,tag=vin.heat_probe,tag=vin.heat_probe_visible,tag=!vin.heat_probe_aggregated] vin.heat_val

# No positive sources remain
execute if score #agg_max vin.warmth_tmp matches 0 run return 1

# Mark every remaining probe tied for the highest value
execute as @e[type=marker,tag=vin.heat_probe,tag=vin.heat_probe_visible,tag=!vin.heat_probe_aggregated] if score @s vin.heat_val = #agg_max vin.warmth_tmp run tag @s add vin.heat_probe_aggregate_candidate
